...
```{r}
# --- Carregue as bibliotecas necessárias antes de usar a função ---
# library(vcfR)
...
drawsp <- function(vcf, 
  ...
  # 1. Converte o VCF para um formato "tidy"
  ...
  # 3. Filtra o SNP de interesse
  dfdat <- vcf_long$dat %>% filter(ID == snp)
  
  # [MELHORIA] Verificação de erro se o SNP não for encontrado
  if (nrow(dfdat) == 0) {
    stop(paste0("ERRO: O SNP '", snp, "' não foi encontrado no arquivo VCF."))
  }
  
  # 4. [MELHORIA] Remove amostras indesejadas pelo nome, de forma segura
  ...
  # 5. Calcula a dose do alelo alternativo
  dfdat$Dose <- factor(
...
map_Cc2 <- map_Cc4 <- map_Ccud <- map_Ccst <- map_Cc
head(map_Cc)
```
...
```{r}
# --- FUNÇÃO MELHORADA E FLEXÍVEL ---

# Carregue as bibliotecas necessárias primeiro
library(dplyr)
library(ggplot2)
library(ggrepel)
library(grafify)

comparar_segregacao <- function(num_amostras,
                                ref_prefix = "Cc",
                                map_updog = NULL, 
                                map_segtest = NULL, 
                                map_fb_2x = NULL, 
  ...
  monomorphic_threshold <- num_amostras * 0.95

  # --- Processa cada dataframe APENAS se ele não for NULL ---

  if (!is.null(map_updog)) {
    processed_maps$updog <- map_updog %>%
      filter(dose_0 < monomorphic_threshold, dose_2 < monomorphic_threshold, dose_4 < monomorphic_threshold) %>%
      select(SNPID, CHR, POS, `1:2:1` = p_val_duplex_1_4_6_4_1) %>%
      mutate(name = "updog")
  }
  
  if (!is.null(map_segtest)) {
    # Supondo que map_segtest já tem as colunas corretas (SNPID, CHR, POS, pval)
    processed_maps$segtest <- map_segtest %>%
      select(SNPID, CHR, POS, `1:2:1` = pval) %>%
      mutate(name = "updog+segtest")
  }
  
  if (!is.null(map_fb_2x)) {
    processed_maps$fb_2x <- map_fb_2x %>%
      filter(`0/0` < monomorphic_threshold, `0/1` < monomorphic_threshold, `1/1` < monomorphic_threshold) %>%
      select(SNPID, CHR, POS, `1:2:1`) %>%
      mutate(name = "FreeBayes 2x")
  }
  
  if (!is.null(map_fb_4x)) {
    processed_maps$fb_4x <- map_fb_4x %>%
      filter(`0/0/0/0` < monomorphic_threshold, `0/0/1/1` < monomorphic_threshold, `1/1/1/1` < monomorphic_threshold) %>%
      select(SNPID, CHR, POS, `1:2:1`) %>%
      mutate(name = "FreeBayes 4x")
  }

  # --- Combina os dataframes que foram processados ---
  if (length(processed_maps) == 0) {
    stop("Nenhum dataframe de entrada foi fornecido.")
  }
  
  map_all <- bind_rows(processed_maps) %>%
    filter(!is.na(`1:2:1`)) %>%
    group_by(name) %>%
    arrange(desc(`1:2:1`)) %>%
    mutate(y_pos = row_number())

  # --- Cálculos para o gráfico (idêntico ao anterior) ---
  bonferroni <- map_all %>% group_by(name) %>% summarise(counts = n(), .groups = 'drop')
  
  distorted <- map_all %>%
    group_by(name) %>%
    summarise(
      total_snps = n(),
      distorted_snps = sum(`1:2:1` < (0.05 / total_snps)),
      dist_percent = (distorted_snps / total_snps) * 100,
      .groups = 'drop'
    )

  # --- Gerar o Gráfico ---
  plot <- ggplot() +
    # ... (código do ggplot é o mesmo da resposta anterior)
    geom_point(data = map_all, aes(x = -log10(`1:2:1`), y = y_pos, color = name), shape = 1, alpha = 0.7) +
    geom_vline(data = bonferroni, aes(xintercept = -log10(0.05 / counts), color = name), 
               linewidth = 0.5, linetype = "dashed") +
    geom_text_repel(
      data = distorted, 
      aes(y = total_snps, x = 1, label = paste0(round(dist_percent, 1), "%")), 
      color = "black", size = 3.5, max.overlaps = 100,
      min.segment.length = 0, nudge_y = 500, show.legend = FALSE
    ) +
    scale_colour_grafify() +
    labs(
      title = paste("Comparação da Segregação de SNPs para a Referência", ref_prefix),
      x = expression(-log[10](italic(P-valor))), y = "Número de SNPs Ordenados", color = "Método"
    ) +
    theme_classic() +
    theme(legend.position = "bottom")
  
  ggsave(
    filename = paste0("grafico_comparacao_chisq_", ref_prefix, ".png"), 
    plot = plot, width = 7, height = 5, dpi = 400
  )
...
```{r}
# --- USO DA FUNÇÃO PARA O DATASET Cc (SEM DADOS 4x) ---

# 1. Prepare os dataframes de entrada que você TEM:
#    - map_Cc (resultado dos seus testes Qui-quadrado no updog)
#    - lrt1 (resultado do segtest)
#    - map_Cc2 (resultado dos testes no FreeBayes 2x)

# Ajuste para o resultado do segtest (lrt1):
map_Ccst_input <- lrt1 %>%
  select(SNPID = snp, pval) %>%
  left_join(map_Cc[, c("SNPID", "CHR", "POS")], by = "SNPID")

# 2. Defina o número de amostras
numero_de_amostras_cc <- 224 # SUBSTITUA PELO SEU NÚMERO REAL

# 3. Chame a função, OMITINDO o argumento map_fb_4x
comparar_segregacao(
  num_amostras = numero_de_amostras_cc,
  ref_prefix = "Cc",
  map_updog = map_Cc, 
  map_segtest = map_Ccst_input,
  map_fb_2x = map_Cc2
  # Note que não precisamos escrever map_fb_4x = NULL, pois já é o padrão
)
```
