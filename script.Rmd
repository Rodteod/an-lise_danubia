...
print("Carregando arquivo GFF...")
gff <- ape::read.gff("Carabica_IGC.gff3.gz")

print("Carregando arquivo FASTA...")
dna <- ape::read.dna("Carabicageisha_871_1.0.fa.gz", format = "fasta")

print("Carregando arquivo VCF...")
vcf_Cc <- read.vcfR("Ce_var_norm_filter_BIAL_QUAL_AF_NS_flex.vcf") 


gff <- gff[which(substr(gff$seqid, 1, 3) == "chr"),]


chr_levels <- paste0("chr", rep(1:11, each=2), c("c","e"))
gff$seqid <- factor(gff$seqid, levels = chr_levels)
gff <- gff[order(gff$seqid), ]

print("Nomes dos cromossomos no VCF (primeiros 6 SNPs):")
print(head(getCHROM(vcf_Cc)))
print("Nomes dos cromossomos no FASTA:")
print(names(dna))
print("Nomes dos cromossomos no GFF (níveis do fator):")
print(levels(gff$seqid))


cromossomo_alvo <- "chr1c"

print(paste("Integrando dados para o cromossomo:", cromossomo_alvo))
chrom <- create.chromR(name=cromossomo_alvo, vcf=vcf_Cc, seq=dna, ann=gff)

print("Gerando gráfico de densidade de variantes...")
plot(chrom)

print("Gerando gráficos de controle de qualidade...")
chromoqc(chrom)
```
...
cat("Resultados de Ce salvos em 'mout_Ca.rda'.\n")
```
...
drawsp <- function(tidy_df, 
  ...
  # Filtra os dados para o SNP de interesse
  dfdat <- tidy_df %>% filter(ID == snp_id)
  
  if (nrow(dfdat) == 0) {
    stop(paste0("ERRO: O SNP '", snp_id, "' não foi encontrado nos dados processados."))
  }
  
  if (!is.null(samples_to_remove)) {
    dfdat <- dfdat %>% filter(!Indiv %in% samples_to_remove)
    if (nrow(dfdat) == 0) {
      stop(paste0("ERRO: Todas as amostras para o SNP '", snp_id, "' foram removidas."))
    }
  }
  
  
  # lapply(sum) soma os números para obter a dose.
  dose_values <- lapply(strsplit(dfdat$gt_GT, "[/|]"), as.integer)
  dfdat$Dose <- factor(
    sapply(dose_values, sum, na.rm = TRUE),
    levels = 0:ploidy,
    ordered = TRUE
  )
  
  #Calcula as linhas de referência para as proporções alélicas esperadas
  pk <- (0:ploidy) / ploidy
  maxcount <- max(c(dfdat$gt_RO, dfdat$gt_AO), na.rm = TRUE) * 1.05 
  slopevec <- pk / (1 - pk)
  # Evita divisão por zero para o caso de dose = ploidy (pk=1)
  slopevec[is.infinite(slopevec)] <- 1e9 
  ...
  # 6. Constrói o gráfico com ggplot2
  final_title <- if (is.null(plot_title)) snp_id else plot_title
  ...
  p <- ggplot(dfdat, aes(x = gt_RO, y = gt_AO)) +
    ...
    geom_point(aes(color = Dose), na.rm = TRUE, size = 1.5, alpha = 0.8) +
    scale_color_manual(values = color_palette, name = "Dose (VCF)", drop = FALSE, na.translate = FALSE) +
    labs(title = final_title, x = "Leituras de Referência (RO)", y = "Leituras Alternativas (AO)") +
    coord_cartesian(xlim = c(0, maxcount), ylim = c(0, maxcount)) + # Garante que as linhas sejam visíveis
...
print("--- Convertendo VCF para formato tidy ---")
vcf_tidy_list <- vcfR2tidy(vcf_Cc_raw, single_frame = TRUE)
vcf_tidy_Cc <- vcf_tidy_list$dat
rm(vcf_tidy_list); gc()

print("--- Adicionando a coluna ID ---")
vcf_tidy_Cc$gt_AO <- as.integer(vcf_tidy_Cc$gt_AO)
vcf_tidy_Cc$gt_RO <- as.integer(vcf_tidy_Cc$gt_RO)
vcf_tidy_Cc$ID <- paste(vcf_tidy_Cc$CHROM, vcf_tidy_Cc$POS, sep = "_")

print("---Verificando a estrutura dos dados processados ---")
print("Nomes das colunas do data.frame 'vcf_tidy_Cc':")
print(colnames(vcf_tidy_Cc))

print("Primeiras linhas do data.frame 'vcf_tidy_Cc':")
print(head(vcf_tidy_Cc[, c("CHROM", "POS", "ID", "Indiv", "gt_RO", "gt_AO")]))
...
#dir.create("diagnostico_snps/melhores_snps_Cc", showWarnings = TRUE, recursive = TRUE)

for (snp_id in best_snps) {
  

  p1 <- drawsp(tidy_df = vcf_tidy_Cc, snp_id = snp_id, ploidy = 4)
  ...
  combined_plot <- ggpubr::ggarrange(p1, p2, ncol = 2, labels = c("VCF Call", "Updog"), common.legend = FALSE, legend = "bottom")
  
  ggsave(filename = paste0("diagnostico_snps/melhores_snps_Cc/", snp_id, "_.png"),
      plot = combined_plot, width = 10, height = 5, dpi = 150)
}

#dir.create("diagnostico_snps/piores_snps_Cc", showWarnings = TRUE, recursive = TRUE)
...
#dir.create("diagnostico_snps/vies_snps_Cc", showWarnings = TRUE, recursive = TRUE)
...
for (snp_id in biased_snps) {
  ...
  ggsave(filename = paste0("diagnostico_snps/vies_snps_Cc/", snp_id, ".png"),
      plot = combined_plot, width = 10, height = 5, dpi = 150)
}
...
for (snp_id in AAaa_outlier) {
  ...
  ggsave(filename = paste0("diagnostico_snps/AAaa_outlier_Cc/", snp_id, "_.png"),
      plot = combined_plot, width = 10, height = 5, dpi = 150)
}
```
...
```{r}
map_Cc2_pval <- map_Cc2 %>%
  select(SNPID, p_val_disomic = `1:2:1`)

# adicionar a coluna de p-valor dissômico ao dataframe 
map_Cc_all_tests <- left_join(map_Cc_all_tests, map_Cc2_pval, by = "SNPID")

head(map_Cc_all_tests)



cat("Iniciando a análise dos modelos de segregação...\n")

#limiar de significância de Bonferroni
bonferroni_threshold <- 0.05 / nrow(map_Cc_all_tests)

# 2. Identifique os SNPs que se ajustam a cada modelo
idx_seg_disomic <- which(map_Cc_all_tests$p_val_disomic > bonferroni_threshold)
idx_seg_tet_sem_dr <- which(map_Cc_all_tests$p_val_tetra_sem_dr > bonferroni_threshold)
idx_seg_tet_com_dr <- which(map_Cc_all_tests$p_val_tetra_com_dr > bonferroni_threshold)

cat("SNPs ajustados ao modelo Dissômico (1:2:1):", length(idx_seg_disomic), "\n")
cat("SNPs ajustados ao modelo Tetra (sem DR):", length(idx_seg_tet_sem_dr), "\n")
cat("SNPs ajustados ao modelo Tetra (com DR):", length(idx_seg_tet_com_dr), "\n")

idx_good_snps <- unique(c(idx_seg_disomic, idx_seg_tet_sem_dr, idx_seg_tet_com_dr))
cat("Total de SNPs com segregação esperada:", length(idx_good_snps), "\n")

df_seg <- map_Cc_all_tests[idx_good_snps, ]


p_val_columns <- c("p_val_disomic", "p_val_tetra_sem_dr", "p_val_tetra_com_dr")


best_model_idx <- apply(df_seg[, p_val_columns], 1, function(row) {
  if (all(is.na(row))) NA else which.max(row)
})


df_seg$SegregationModel <- p_val_columns[best_model_idx]

df_seg$SegregationModel <- case_when(
  df_seg$SegregationModel == "p_val_disomic" ~ "Dissômico (1:2:1)",
  df_seg$SegregationModel == "p_val_tetra_sem_dr" ~ "Tetrassômico (sem DR)",
  df_seg$SegregationModel == "p_val_tetra_com_dr" ~ "Tetrassômico (com DR)",
  TRUE ~ NA_character_
)

df_plot <- df_seg %>%
  filter(!is.na(SegregationModel)) %>% # Remove SNPs onde nenhum modelo pôde ser determinado
  count(CHR, SegregationModel) %>%
  rename(Chromosome = CHR, Frequency = n)

# 6. Gere o Gráfico
library(ggplot2)
library(scales)

ggplot(df_plot, aes(x = Chromosome, y = Frequency, fill = SegregationModel)) +
  geom_bar(position = "fill", stat = "identity") +
  scale_y_continuous(labels = percent, expand = c(0, 0)) +
  scale_fill_manual(values = c("Dissômico (1:2:1)" = "#56B4E9", 
                               "Tetrassômico (sem DR)" = "#009E73",
                               "Tetrassômico (com DR)" = "#E69F00")) +
  labs(
    title = "Proporção de Modelos de Segregação por Cromossomo (Subgenoma Cc)",
    subtitle = "Apenas para SNPs com segregação esperada",
    x = "Cromossomo",
    y = "Proporção de SNPs (%)",
    fill = "Modelo de Segregação Predominante"
  ) +
  theme_classic() +
  theme(
...
# Salve o gráfico
ggsave(
  filename = paste0("grafico_segregacao_3modelos_", "Cc", ".png"), 
...
map_Cc_all_tests <- map_Cc 

#Extrair p-valores do dataframe do updog (map_Ccud)
pvals_updog <- map_Ccud %>%
  select(
    SNPID,
    p_val_updog_disomic = `1:2:1`,
    p_val_updog_tet_sem_dr = `1:4:6:4:1`,
    p_val_updog_tet_com_dr = `1:8:18:8:1`
  )

#Extrair p-valores do dataframe do FreeBayes (map_Cc2)
pvals_freebayes <- map_Cc2 %>%
  select(
    SNPID,
    p_val_freebayes_disomic = `1:2:1`
  )
  
#Extrair p-valores do dataframe do segtest (map_Ccst)
pvals_segtest <- map_Ccst %>%
  select(
    SNPID,
    p_val_segtest = `p_value.x`#usar crase se tiver um ponto no nome
  )

map_Cc_all_tests <- map_Cc_all_tests %>%
  left_join(pvals_updog, by = "SNPID") %>%
  left_join(pvals_freebayes, by = "SNPID") %>%
  left_join(pvals_segtest, by = "SNPID")

print(colnames(map_Cc_all_tests))
print(head(map_Cc_all_tests))

save(map_Cc_all_tests, file = "map_Cc_all_tests.rda")
```
```{r}

# load("map_Cc_all_tests.rda")


p_val_columns <- c("p_val_updog_disomic", "p_val_updog_tet_sem_dr", "p_val_updog_tet_com_dr")

if (!all(p_val_columns %in% colnames(map_Cc_all_tests))) {
  stop("ERRO: Uma ou mais colunas  não foram encontradas no dataframe.")
}

print(summary(map_Cc_all_tests[, p_val_columns]))

threshold_usado <- 0.05 / nrow(map_Cc_all_tests) # Limiar de Bonferroni
cat("\nUsando um limiar de p-valor de:", threshold_usado, "\n")

idx_good_snps <- map_Cc_all_tests %>%
  filter(
    p_val_updog_disomic > threshold_usado |
    p_val_updog_tet_sem_dr > threshold_usado |
    p_val_updog_tet_com_dr > threshold_usado
  )


if (nrow(idx_good_snps) > 0) {
  
  df_seg <- idx_good_snps
  
  best_model_indices <- apply(df_seg[, p_val_columns], 1, which.max)
  df_seg$SegregationModel <- p_val_columns[best_model_indices]
  
  df_seg$SegregationModel <- case_when(
    df_seg$SegregationModel == "p_val_updog_disomic" ~ "Dissômico (1:2:1)",
    df_seg$SegregationModel == "p_val_updog_tet_sem_dr" ~ "Tetrassômico (sem DR)",
    df_seg$SegregationModel == "p_val_updog_tet_com_dr" ~ "Tetrassômico (com DR)",
    TRUE ~ NA_character_
  )
  ...
  meu_grafico <- ggplot(df_plot, aes(x = Chromosome, y = Frequency, fill = SegregationModel)) +
    geom_bar(position = "fill", stat = "identity") +
    scale_y_continuous(labels = percent, expand = c(0, 0)) +
    scale_fill_manual(
      values = c("Dissômico (1:2:1)" = "#56B4E9", 
                 "Tetrassômico (sem DR)" = "#009E73", 
                 "Tetrassômico (com DR)" = "#E69F00"),
      name = "Modelo de Segregação (Updog)"
    ) +
    labs(
      title = "Proporção de Modelos de Segregação por Cromossomo",
      subtitle = paste("Baseado em", nrow(df_seg), "SNPs com p-valor >", format(threshold_usado, scientific = TRUE, digits = 2)),
      x = "Cromossomo", y = "Proporção de SNPs (%)"
    ) +
    theme_classic() +
    theme(legend.position = "bottom")
  
  print(meu_grafico)
  ggsave("grafico_segregacao.png", plot = meu_grafico, width = 8, height = 6, dpi = 400)
...
load("snp_Ca.rda"); cat("Dimensões iniciais snp_Ca (SNPs x Amostras):", dim(snp_Ca), "\n")
load("snp_Cc.rda"); cat("Dimensões iniciais snp_Cc (SNPs x Amostras):", dim(snp_Cc), "\n")
load("snp_Ce.rda"); cat("Dimensões iniciais snp_Ce (SNPs x Amostras):", dim(snp_Ce), "\n")

# Transpõe as matrizes 
snp_Ca <- t(snp_Ca)
snp_Cc <- t(snp_Cc)
snp_Ce <- t(snp_Ce)
cat("\nMatrizes transpostas para (Amostras x SNPs) para filtragem.\n\n")


cat("-- Marcadores com >25% de dados faltantes ---\n")
exclude.mrk_Ca <- marker.callrate(snp_Ca); cat("Marcadores a remover de Ca:", length(exclude.mrk_Ca), "\n")
exclude.mrk_Cc <- marker.callrate(snp_Cc); cat("Marcadores a remover de Cc:", length(exclude.mrk_Cc), "\n")
exclude.mrk_Ce <- marker.callrate(snp_Ce); cat("Marcadores a remover de Ce:", length(exclude.mrk_Ce), "\n")

if (length(exclude.mrk_Ca) > 0) snp_Ca <- snp_Ca[, -exclude.mrk_Ca]
if (length(exclude.mrk_Cc) > 0) snp_Cc <- snp_Cc[, -exclude.mrk_Cc]
if (length(exclude.mrk_Ce) > 0) snp_Ce <- snp_Ce[, -exclude.mrk_Ce]
cat("Dimensões após filtro de marcadores: Ca:", dim(snp_Ca), "| Cc:", dim(snp_Cc), "| Ce:", dim(snp_Ce), "\n\n")


cat("--- Filtro 2: Indivíduos com >25% de dados faltantes ---\n")
exclude.ind_Ca <- ind.callrate(snp_Ca); cat("Indivíduos a remover de Ca:", length(exclude.ind_Ca), "\n")
exclude.ind_Cc <- ind.callrate(snp_Cc); cat("Indivíduos a remover de Cc:", length(exclude.ind_Cc), "\n")
exclude.ind_Ce <- ind.callrate(snp_Ce); cat("Indivíduos a remover de Ce:", length(exclude.ind_Ce), "\n")

if (length(exclude.ind_Ca) > 0) snp_Ca <- snp_Ca[-exclude.ind_Ca, ]
if (length(exclude.ind_Cc) > 0) snp_Cc <- snp_Cc[-exclude.ind_Cc, ]
if (length(exclude.ind_Ce) > 0) snp_Ce <- snp_Ce[-exclude.ind_Ce, ] # Corrigido de _Ca para _Ce
cat("Dimensões após filtro de indivíduos: Ca:", dim(snp_Ca), "| Cc:", dim(snp_Cc), "| Ce:", dim(snp_Ce), "\n\n")


cat("--- Filtro 3: Marcadores com frequência de classe >75% ---\n")
exclude.class_Ca <- class.freq(snp_Ca); cat("Marcadores a remover de Ca:", length(exclude.class_Ca), "\n")
exclude.class_Cc <- class.freq(snp_Cc); cat("Marcadores a remover de Cc:", length(exclude.class_Cc), "\n")
exclude.class_Ce <- class.freq(snp_Ce); cat("Marcadores a remover de Ce:", length(exclude.class_Ce), "\n")

if (length(exclude.class_Ca) > 0) snp_Ca <- snp_Ca[, -exclude.class_Ca]
if (length(exclude.class_Cc) > 0) snp_Cc <- snp_Cc[, -exclude.class_Cc]
if (length(exclude.class_Ce) > 0) snp_Ce <- snp_Ce[, -exclude.class_Ce]
cat("Dimensões após filtro de classe: Ca:", dim(snp_Ca), "| Cc:", dim(snp_Cc), "| Ce:", dim(snp_Ce), "\n\n")


cat("--- Filtro 4: Marcadores com MAF <25% ---\n")
exclude.maf_Ca <- allele.freq(snp_Ca); cat("Marcadores a remover de Ca:", length(exclude.maf_Ca), "\n")
exclude.maf_Cc <- allele.freq(snp_Cc); cat("Marcadores a remover de Cc:", length(exclude.maf_Cc), "\n")
exclude.maf_Ce <- allele.freq(snp_Ce); cat("Marcadores a remover de Ce:", length(exclude.maf_Ce), "\n")

if (length(exclude.maf_Ca) > 0) snp_Ca <- snp_Ca[, -exclude.maf_Ca]
if (length(exclude.maf_Cc) > 0) snp_Cc <- snp_Cc[, -exclude.maf_Cc]
if (length(exclude.maf_Ce) > 0) snp_Ce <- snp_Ce[, -exclude.maf_Ce]
cat("Dimensões após filtro de MAF: Ca:", dim(snp_Ca), "| Cc:", dim(snp_Cc), "| Ce:", dim(snp_Ce), "\n\n")


cat("--- Filtro 5: Marcadores redundantes ---\n")
snp_Ca <- t(snp_Ca)
snp_Ca <- snp_Ca[!duplicated(snp_Ca), ]
cat("Dimensões finais snp_Ca (SNPs x Amostras):", dim(snp_Ca), "\n")
print(table(gsub("chr|_.*", "", rownames(snp_Ca))))

snp_Cc <- t(snp_Cc)
snp_Cc <- snp_Cc[!duplicated(snp_Cc), ]
cat("Dimensões snp_Cc (SNPs x Amostras):", dim(snp_Cc), "\n")
print(table(gsub("chr|_.*", "", rownames(snp_Cc))))

snp_Ce <- t(snp_Ce)
snp_Ce <- snp_Ce[!duplicated(snp_Ce), ]
cat("Dimensões snp_Ce (SNPs x Amostras):", dim(snp_Ce), "\n")
print(table(gsub("chr|_.*", "", rownames(snp_Ce))))


save(snp_Ca, file = "snp_Ca_filtrado.rda")
save(snp_Cc, file = "snp_Cc_filtrado.rda")
save(snp_Ce, file = "snp_Ce_filtrado.rda")

cat("\nMatrizes filtradas foram salvas em arquivos .rda.\n")
```

```{r plot_kept_snps, cache=FALSE}
load("snp_Cc_filtrado.rda"); cat("Carregados", nrow(snp_Cc), "SNPs filtrados para Cc.\n")
load("snp_Ce_filtrado.rda"); cat("Carregados", nrow(snp_Ce), "SNPs filtrados para Ce.\n")

load("mout_Cc.rda")
load("mout_Ce.rda")

indices_Cc <- which(mout_Cc$snpdf$snp %in% rownames(snp_Cc))
indices_Ce <- which(mout_Ce$snpdf$snp %in% rownames(snp_Ce))
cat("Encontrados os índices de", length(indices_Cc), "SNPs mantidos para Cc.\n")
cat("Encontrados os índices de", length(indices_Ce), "SNPs mantidos para Ce.\n")

dir.create(path = "plots_filtrados/plots_Cc", showWarnings = FALSE, recursive = TRUE)
dir.create(path = "plots_filtrados/plots_Ce", showWarnings = FALSE, recursive = TRUE)


cat("\nIniciando a criação de plots para os SNPs filtrados de Cc...\n")

blocos_indices_Cc <- split(indices_Cc, ceiling(seq_along(indices_Cc) / 100))

for (i in 1:length(blocos_indices_Cc)) {
  pdf_path <- paste0("plots_filtrados/plots_Cc/mout_kept_Cc_bloco_", i, ".pdf")
  pdf(pdf_path, width = 12, height = 10) # Aumentado para caber mais plots por página
  
  print(plot(mout_Cc, indices = blocos_indices_Cc[[i]]))
  
  # Fecha o arquivo PDF
  dev.off()
  cat("  -> Gerado o arquivo:", pdf_path, "\n")
}
cat("Plots para Cc concluídos.\n")


cat("\nIniciando a criação de plots para os SNPs filtrados de Ce...\n")
blocos_indices_Ce <- split(indices_Ce, ceiling(seq_along(indices_Ce) / 100))

for (i in 1:length(blocos_indices_Ce)) {
  pdf_path <- paste0("plots_filtrados/plots_Ce/mout_kept_Ce_bloco_", i, ".pdf")
  pdf(pdf_path, width = 12, height = 10)
  
  print(plot(mout_Ce, indices = blocos_indices_Ce[[i]]))
  
  dev.off()
  cat("  -> Gerado o arquivo:", pdf_path, "\n")
}
cat("Plots para Ce concluídos.\n")
```

```{r create_map_files, cache=FALSE}

load("snp_Cc_filtered.rda")
load("snp_Ce_filtered.rda")

# Verifique se o pacote 'tidyr' está instalado
if (!require("tidyr")) install.packages("tidyr")
library(tidyr)
library(dplyr) 

cat("--- Criando arquivos de mapa físico ---\n")

cat("\nProcessando 'Cc'...\n")

map_Cc <- data.frame(SNPID = colnames(snp_Cc))
...
map_Cc <- map_Cc %>%
  separate(
    col = SNPID,
    into = c("CHR", "POS"),
    sep = "_", # O caractere que separa as colunas
    remove = FALSE, # Mantém a coluna original SNPID
    convert = TRUE # converter automaticamente CHR para texto e POS para número
  )

map_Cc$CHR <- gsub("chr|c", "", map_Cc$CHR)


ordered_levels_Cc <- unique(map_Cc$CHR) %>% as.numeric() %>% sort() %>% as.character()

map_Cc$CHR <- factor(map_Cc$CHR, levels = ordered_levels_Cc)

cat("Dimensões do mapa 'Cc':", dim(map_Cc), "\n")
head(map_Cc)
cat("Ordem dos cromossomos em 'Cc':", levels(map_Cc$CHR), "\n")


cat("\nProcessando 'Ce'...\n")
map_Ce <- data.frame(SNPID = colnames(snp_Ce)) %>%
  separate(SNPID, into = c("CHR", "POS"), sep = "_", remove = FALSE, convert = TRUE)

map_Ce$CHR <- gsub("chr|e", "", map_Ce$CHR)

ordered_levels_Ce <- unique(map_Ce$CHR) %>% as.numeric() %>% sort() %>% as.character()
map_Ce$CHR <- factor(map_Ce$CHR, levels = ordered_levels_Ce)

cat("Dimensões do mapa 'Ce':", dim(map_Ce), "\n")
head(map_Ce)
cat("Ordem dos cromossomos em 'Ce':", levels(map_Ce$CHR), "\n")



save(snp_Cc, map_Cc, file = "snp_filtered_Cc.rda")
save(snp_Ce, map_Ce, file = "snp_filtered_Ce.rda")

cat("\nMatrizes de dosagens e mapas físicos foram salvos com sucesso!\n")
```

```{r}
# --- CARREGAR OS DADOS FILTRADOS DO PASSO ANTERIOR ---
# Carregamos os objetos salvos no final da pipeline de filtragem.
load("snp_Cc_filtrado.rda"); cat("Carregada matriz snp_Cc com dimensões:", dim(snp_Cc), "\n")
load("snp_Ce_filtrado.rda"); cat("Carregada matriz snp_Ce com dimensões:", dim(snp_Ce), "\n")


# --- CRIAR O MAPA FÍSICO PARA O SUBGENOMA Cc ---
# O erro foi corrigido aqui: usamos rownames(snp_Cc) para obter os IDs dos SNPs.
map_Cc <- data.frame(
  SNPID = rownames(snp_Cc),
  matrix(unlist(strsplit(rownames(snp_Cc), split = "_")), ncol = 2, byrow = TRUE, dimnames = list(NULL, c("CHR", "POS")))
)

# --- LIMPEZA E FORMATAÇÃO DAS COLUNAS DO MAPA Cc ---
# Remove os prefixos "chr" e os sufixos de subgenoma "c"
map_Cc$CHR <- gsub("chr", "", map_Cc$CHR)
map_Cc$CHR <- gsub("c", "", map_Cc$CHR)

# Converte a coluna de posição para número inteiro
map_Cc$POS <- as.integer(map_Cc$POS)

# Boa prática: Converte a coluna de cromossomo em um fator com níveis ordenados numericamente.
# Isso garante que os cromossomos sempre apareçam na ordem correta (1, 2, 3, ..., 11) em gráficos futuros.
map_Cc$CHR <- factor(map_Cc$CHR, levels = sort(as.numeric(unique(map_Cc$CHR))))

cat("\nMapa físico para Cc criado com sucesso.\n")
print(head(map_Cc))


# --- CRIAR O MAPA FÍSICO PARA O SUBGENOMA Ce ---
# O mesmo erro foi corrigido aqui: usamos rownames(snp_Ce).
map_Ce <- data.frame(
  SNPID = rownames(snp_Ce),
  matrix(unlist(strsplit(rownames(snp_Ce), split = "_")), ncol = 2, byrow = TRUE, dimnames = list(NULL, c("CHR", "POS")))
)

# --- LIMPEZA E FORMATAÇÃO DAS COLUNAS DO MAPA Ce ---
map_Ce$CHR <- gsub("chr", "", map_Ce$CHR)
map_Ce$CHR <- gsub("e", "", map_Ce$CHR)
map_Ce$POS <- as.integer(map_Ce$POS)
map_Ce$CHR <- factor(map_Ce$CHR, levels = sort(as.numeric(unique(map_Ce$CHR))))

cat("\nMapa físico para Ce criado com sucesso.\n")
print(head(map_Ce))


# --- SALVAR OS OBJETOS FINAIS PARA A CONSTRUÇÃO DO MAPA DE LIGAÇÃO ---
# Salvamos tanto a matriz de dosagem final quanto o dataframe de mapa correspondente no mesmo arquivo.
save(snp_Cc, map_Cc, file = "dados_mapeamento_Cc.rda")
save(snp_Ce, map_Ce, file = "dados_mapeamento_Ce.rda")

cat("\nMatrizes de dosagem e dataframes de mapa foram salvos em 'dados_mapeamento_Cc.rda' e 'dados_mapeamento_Ce.rda'.\n")

```

```{r}
# --- SOLUÇÃO: Criar tabela de tamanhos a partir do objeto DNA já carregado ---

# Verifica se o objeto 'dna' existe. Se não, recarregue-o como fez no início do script.
if (!exists("dna")) {
  stop("O objeto 'dna' não foi encontrado. Por favor, rode o bloco inicial onde você usa read.dna()")
}

# Calcula o comprimento de cada sequência no objeto DNA
chr_lengths <- sapply(dna, length)

# Cria o data frame 'sizes' com o formato que o código precisa
sizes <- data.frame(
  CHR = names(dna),
  LENGTH = chr_lengths
)

# Remove "sizes.genome" da lógica e usa o objeto criado internamente
# (O resto do código abaixo continua igual, apenas removemos o read.table)
```


```{r plot_snp_density, cache=FALSE, warning=FALSE}
load("snp_filtered_Cc.rda")
load("snp_filtered_Ce.rda")
sizes <- read.table("sizes.genome")
colnames(sizes) <- c("CHR","POS")
sizes$CHR <- gsub(pattern = "chr", replacement = "", sizes$CHR)
sizes$SNPID <- paste0("chr", sizes$CHR, "_", sizes$POS)
map_Cc <- rbind(map_Cc, sizes[seq(1,nrow(sizes),2),])
map_Cc$CHR <- factor(map_Cc$CHR, levels = sizes$CHR[seq(1,nrow(sizes),2)])
map_Ce <- rbind(map_Ce, sizes[seq(2,nrow(sizes),2),])
map_Ce$CHR <- factor(map_Ce$CHR, levels = sizes$CHR[seq(2,nrow(sizes),2)])
cyto_Cc <- CMplot(map_Cc, type = "p", plot.type = "d", bin.size = 1e6, bin.breaks = seq(0, 50, 5), chr.den.col = c("darkgreen", "yellow", "red"), verbose = FALSE, chr.pos.max = TRUE, file.output = TRUE, file = "pdf", file.name = "Cc")
cyto_Ce <- CMplot(map_Ce, type = "p", plot.type = "d", bin.size = 1e6, bin.breaks = seq(0, 50, 5), chr.den.col = c("darkgreen", "yellow", "red"), verbose = FALSE, chr.pos.max = TRUE, file.output = TRUE, file = "pdf", file.name = "Ce")
