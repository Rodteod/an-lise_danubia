...
# Use left_join para adicionar a coluna de p-valor dissômico ao dataframe mestre
map_Cc_all_tests <- left_join(map_Cc_all_tests, map_Cc2_pval, by = "SNPID")

# Verifique o resultado
cat("Dataframe mestre criado com sucesso. Dimensões:", dim(map_Cc_all_tests), "\n")
head(map_Cc_all_tests)


# --- ETAPA 3: ANÁLISE E PLOTAGEM (SEU ÚLTIMO BLOCO, AGORA CORRIGIDO) ---

cat("Iniciando a análise dos modelos de segregação...\n")

# 1. Defina o limiar de significância de Bonferroni
bonferroni_threshold <- 0.05 / nrow(map_Cc_all_tests)

# 2. Identifique os SNPs que se ajustam a cada modelo
#    Filtramos NAs para evitar erros nos índices
...
# 3. Crie um conjunto com TODOS os SNPs que passaram em PELO MENOS UM dos testes
...
# Crie o dataframe de segregação usando apenas esses SNPs de alta qualidade
df_seg <- map_Cc_all_tests[idx_good_snps, ]

# 4. Determine o "melhor" modelo para cada SNP
#    Agora os nomes das colunas são os que definimos acima
p_val_columns <- c("p_val_disomic", "p_val_tetra_sem_dr", "p_val_tetra_com_dr")

# O apply() pode retornar NAs se todas as colunas de p-valor forem NA para uma linha.
# Adicionamos na.rm = TRUE para lidar com isso
best_model_idx <- apply(df_seg[, p_val_columns], 1, function(row) {
  if (all(is.na(row))) NA else which.max(row)
})


# Atribua o nome do melhor modelo
df_seg$SegregationModel <- p_val_columns[best_model_idx]

# Deixe os nomes mais amigáveis para a legenda
...
# 5. Prepare os dados para o gráfico
df_plot <- df_seg %>%
...
```{r}
f2 <- which(map_Ccud$`1:2:1` > 0.05/nrow(map_Ccud)); length(f2)
f2allo <- which(map_Ccud$`1:4:6:4:1` > 0.05/nrow(map_Ccud)); length(f2allo)
f2auto <- which(map_Ccud$`1:8:18:8:1` > 0.05/nrow(map_Ccud)); length(f2auto)
length(unique(c(f2, f2allo, f2auto)))

df_seg <- map_Ccud[f2,]
df_seg$Segregation <- apply(df_seg[,9:11], 1, which.max)
df_seg$Segregation <- factor(df_seg$Segregation, labels = colnames(df_seg)[9:11])
df_seg <- data.frame(table(df_seg$CHR, df_seg$Segregation))
ggplot(df_seg, aes(x=Var1, y = Freq, fill = Var2)) +
  geom_bar(position="fill", stat="identity") +
  scale_y_continuous(labels = function(x) x*100, expand = c(0,0)) +
  labs(y = "Frequency (%)", alpha = "Number of\nclasses") +
  theme_classic() +
  theme(strip.background = element_blank())
ggsave("segr.png", width = 7, height = 4, dpi = 400)
```
...
cat("--- Filtragem da Matriz de Dosagens ---\n")

cat("\nProcessando a matriz 'Ca'...\n")

snp_Ca <- t(snp_Ca)
cat("Dimensões ANTES da filtragem (Amostras x Marcadores):", dim(snp_Ca), "\n")


exclude_mrk_Ca <- marker.callrate(snp_Ca)
cat(length(exclude_mrk_Ca), "marcadores em 'Ca' serão removidos (>25% de dados faltantes).\n")
if (length(exclude_mrk_Ca) > 0) {
  snp_Ca <- snp_Ca[, -exclude_mrk_Ca]
}
cat("Dimensões DEPOIS da filtragem (Amostras x Marcadores):", dim(snp_Ca), "\n")


cat("\nProcessando a matriz 'Cc'...\n")
snp_Cc <- t(snp_Cc)
cat("Dimensões ANTES da filtragem (Amostras x Marcadores):", dim(snp_Cc), "\n")
exclude_mrk_Cc <- marker.callrate(snp_Cc)
cat(length(exclude_mrk_Cc), "marcadores em 'Cc' serão removidos (>25% de dados faltantes).\n")
if (length(exclude_mrk_Cc) > 0) {
  snp_Cc <- snp_Cc[, -exclude_mrk_Cc]
}
cat("Dimensões DEPOIS da filtragem (Amostras x Marcadores):", dim(snp_Cc), "\n")


cat("\nProcessando a matriz 'Ce'...\n")
snp_Ce <- t(snp_Ce)
cat("Dimensões ANTES da filtragem (Amostras x Marcadores):", dim(snp_Ce), "\n")
exclude_mrk_Ce <- marker.callrate(snp_Ce)
cat(length(exclude_mrk_Ce), "marcadores em 'Ce' serão removidos (>25% de dados faltantes).\n")
if (length(exclude_mrk_Ce) > 0) {
  snp_Ce <- snp_Ce[, -exclude_mrk_Ce]
}
cat("Dimensões DEPOIS da filtragem (Amostras x Marcadores):", dim(snp_Ce), "\n")

save(snp_Ca, file = "snp_Ca_filtered.rda")
save(snp_Cc, file = "snp_Cc_filtered.rda")
save(snp_Ce, file = "snp_Ce_filtered.rda")
```

```{r plot_kept_snps, cache=FALSE}
# Carregue os objetos necessários, se ainda não estiverem no ambiente.
load("snp_Cc_filtered.rda") # Use a matriz já filtrada da etapa anterior
load("mout_Cc.rda")
load("snp_Ce_filtered.rda") # Use a matriz já filtrada da etapa anterior
load("mout_Ce.rda")

cat("--- Gerando gráficos de diagnóstico para os SNPs mantidos ---\n")

# --- Processamento para o alinhamento Canephora (Cc) ---
cat("\nProcessando SNPs de 'Cc'...\n")

# Passo 1: Identificar os índices dos SNPs mantidos dentro do objeto 'mout_Cc'
# Note que `snp_Cc` deve ter SNPs nas colunas, como na etapa anterior.
# Se você não transpôs `snp_Cc` e a salvou, use `rownames()` em vez de `colnames()`.
# Assumindo que você está usando a matriz transposta (Amostras x SNPs):
snps_mantidos_Cc <- colnames(snp_Cc)
indices_Cc <- which(mout_Cc$snpdf$snp %in% snps_mantidos_Cc)
cat("Encontrados", length(indices_Cc), "SNPs para plotar em 'Cc'.\n")

# Passo 2: Dividir os índices em blocos de 100 para criar múltiplos arquivos PDF.
# A função split cria uma lista, onde cada elemento da lista é um vetor com até 100 índices.
indices_list_Cc <- split(indices_Cc, ceiling(seq_along(indices_Cc) / 100))
cat("Os SNPs de 'Cc' serão salvos em", length(indices_list_Cc), "arquivos PDF.\n")

# Passo 3: Criar um diretório para os plots dos SNPs mantidos.
dir.create(path = "plots_kept_snps_Cc", showWarnings = FALSE)

# Passo 4: Loop para gerar um PDF para cada bloco de 100 SNPs.
for (i in seq_along(indices_list_Cc)) {
  # Define o nome do arquivo PDF dinamicamente.
  pdf_file_name <- paste0("plots_kept_snps_Cc/mout_kept_Cc_chunk_", i, ".pdf")
  pdf(pdf_file_name, width = 8, height = 6)
  
  # Plota o subconjunto de SNPs correspondente a este chunk.
  # A função plot() do updog organiza os gráficos automaticamente.
  print(plot(mout_Cc, indices = indices_list_Cc[[i]]))
  
  # Fecha o arquivo PDF atual antes de ir para o próximo.
  dev.off()
}
cat("Gráficos para 'Cc' gerados com sucesso!\n")


# --- Repetir o mesmo processo para o alinhamento Eugenioides (Ce) ---
cat("\nProcessando SNPs de 'Ce'...\n")
snps_mantidos_Ce <- colnames(snp_Ce)
indices_Ce <- which(mout_Ce$snpdf$snp %in% snps_mantidos_Ce)
cat("Encontrados", length(indices_Ce), "SNPs para plotar em 'Ce'.\n")

indices_list_Ce <- split(indices_Ce, ceiling(seq_along(indices_Ce) / 100))
cat("Os SNPs de 'Ce' serão salvos em", length(indices_list_Ce), "arquivos PDF.\n")

dir.create(path = "plots_kept_snps_Ce", showWarnings = FALSE)

for (i in seq_along(indices_list_Ce)) {
  pdf_file_name <- paste0("plots_kept_snps_Ce/mout_kept_Ce_chunk_", i, ".pdf")
  pdf(pdf_file_name, width = 8, height = 6)
  print(plot(mout_Ce, indices = indices_list_Ce[[i]]))
  dev.off()
}
cat("Gráficos para 'Ce' gerados com sucesso!\n")

```

```{r create_map_files, cache=FALSE}
# Carregue as matrizes de dosagens filtradas (Amostras x SNPs)
# Usando os arquivos salvos na etapa anterior
...
library(tidyr)
library(dplyr) # tidyr funciona melhor com dplyr
...
# O ponto de partida é um data frame apenas com os nomes dos SNPs
map_Cc <- data.frame(SNPID = colnames(snp_Cc))

# Usamos a função separate() do tidyr para dividir a coluna SNPID.
# É muito mais legível que a combinação strsplit/unlist/matrix.
map_Cc <- map_Cc %>%
  separate(
    col = SNPID,
    into = c("CHR", "POS"),
    ...
    convert = TRUE # Tenta converter automaticamente CHR para texto e POS para número
  )

# Limpeza do nome do cromossomo
map_Cc$CHR <- gsub("chr|c", "", map_Cc$CHR)

# CORREÇÃO CRÍTICA: Ordenar os cromossomos numericamente
# Primeiro, pegamos os nomes únicos dos cromossomos
# Depois, convertemos para número e ordenamos, e então de volta para caractere.
ordered_levels_Cc <- unique(map_Cc$CHR) %>% as.numeric() %>% sort() %>% as.character()

# Aplicamos essa ordem correta ao criar o fator
...
# --- Repetir o processo para o alinhamento Eugenioides (Ce) ---
cat("\nProcessando 'Ce'...\n")
...
# Salva a matriz de genótipos JUNTO com seu mapa correspondente.
