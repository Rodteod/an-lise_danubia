...
```{r}
drawsp <- function(vcf, snp, ploidy = 4, sec_axis = NULL, color = col_pal, title = "") {
  pk <- (0:ploidy)/ploidy
  ...
  dfdat <- dfdat[-c(227:235),]
  dfdat$Dose <- factor(unlist(lapply(lapply(strsplit(dfdat$gt_GT, "/"), as.integer), sum)), ordered = TRUE)
  maxcount <- max(max(dfdat$gt_RO, na.rm = TRUE), max(dfdat$gt_AO, na.rm = TRUE)) + 1
  slopevec <- pk/(1 - pk)
  ...
  p <- ggplot(dfdat) +
    ...
    # labs(title = ifelse(isTRUE(title), snp, ""), x = ifelse(isTRUE(x_axis), "Counts a", ""), y = ifelse(isTRUE(y_axis), "Counts A", "")) + 
    theme_bw() + 
    theme(plot.title = element_text(size=10, hjust = c(.5,.5)), axis.title.x = element_blank(), axis.title.y = element_blank())#, axis.title = element_text(size=9))
  return(p)
}
```
...
for (snp_id in best_snps) {
 ...
 p2 <- updog:::plot.multidog(mout_Cc, indices = which(mout_Cc$snpdf$snp == snp_id))[[1]] +
  ...
  combined_plot <- ggpubr::ggarrange(p1, p2, ncol = 2, labels = c("VCF Bruto", "Updog"), common.legend = FALSE, legend = "bottom")
...
p2 <- plot(mout_Cc, indices = which(mout_Cc$snpdf$snp == snp_id))[[1]] +
      scale_color_manual(values = col_pal, drop = FALSE, na.translate = FALSE) +
      labs(color = "Dose (updog)", alpha = "Max. post. prob.") +
      theme(legend.position="top", plot.title = element_blank(), axis.title = element_blank())
